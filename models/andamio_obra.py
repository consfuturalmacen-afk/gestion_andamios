...
class AndamioObra(models.Model):
    ...
    @api.depends("movimiento_ids.estado", "movimiento_ids.tipo_movimiento", "movimiento_ids.lineas_ids.cantidad", "movimiento_ids.lineas_ids.pieza_id")
    def _compute_resumen_obras(self):
        for obra in self:
            acumulado = defaultdict(float)
            for movimiento in obra.movimiento_ids.filtered(lambda m: m.estado in ("en_obra", "devuelto")):
            ...
            obra.piezas_en_obra = (
                ", ".join(f"{pieza}: {cantidad:g}" for pieza, cantidad in sorted(acumulado.items()))
                if acumulado
